...
- Dashboard totals for projects, tasks, completed work, overdue tasks, and per-project assignment/progress counts
- REST APIs backed by SQLite relationships and validation

The frontend files are kept at the repo root (`index.html`, `styles.css`, and `app.js`) so the project is easy to upload through GitHub's web UI.
...
Tasks can only be assigned to users who are already members of the selected project. Project overview shows total tasks, assigned tasks, unassigned tasks, members, and completion percentage.

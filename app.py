...
class Handler(SimpleHTTPRequestHandler):
        ...
        try:
            with db() as conn:
                ...
                if path.startswith("/api/projects/") and method == "DELETE":
                    return self.delete_project(conn, user, path)
    ...
    def do_GET(self):
        ...
        if static_path not in {"/index.html", "/styles.css", "/app.js"}:
            return self.error_json(HTTPStatus.NOT_FOUND, "File not found.")
        return super().do_GET()
    ...
    def projects(self, conn, user):
        if user["role"] == "admin":
            rows = conn.execute(
                ...
                COUNT(DISTINCT CASE WHEN t.assignee_id IS NOT NULL THEN t.id END) AS assigned_task_count,
                COUNT(DISTINCT CASE WHEN t.assignee_id IS NULL THEN t.id END) AS unassigned_task_count,
    ...
    def delete_project(self, conn, user, path):
        if user["role"] != "admin":
            return self.error_json(HTTPStatus.FORBIDDEN, "Only admins can delete projects.")
        parts = path.split("/")
        if len(parts) != 4:
            return self.error_json(HTTPStatus.NOT_FOUND, "Endpoint not found.")
        project_id = int(parts[3])
        ...
        if not project:
            return self.error_json(HTTPStatus.NOT_FOUND, "Project not found.")

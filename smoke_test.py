...
def main():
    ...
    try:
        ...
        request("DELETE", f"/api/projects/{project['id']}", admin_token)
        dashboard = request("GET", "/api/dashboard", admin_token)
        assert dashboard["projects"] == 0
        assert dashboard["tasks"] == 0

        print("Smoke test passed: auth, RBAC, projects, members, tasks, dashboard, and project delete work.")
    finally:
        server.shutdown()
        server.server_close()
